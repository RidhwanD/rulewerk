...
public class Experiment {
	public static void main(String[] arg) throws IOException, ParsingException {
		// java Experiment -ver -size -iter -c -d -c -q -suffix
		int ver = Integer.valueOf(arg[0]);
		int size = Integer.valueOf(arg[1]);
		int iter = Integer.valueOf(arg[2]);
		int i = Integer.valueOf(arg[3]);
		int j = Integer.valueOf(arg[4]);
		int k = Integer.valueOf(arg[5]);
		int l = Integer.valueOf(arg[6]);
		String suffix = arg[7];
		
		ReasoningUtils.configureLogging(); // use simple logger for the example
		
		File file = new File(ExamplesUtils.OUTPUT_FOLDER + "rpq/new2/new-"+ver+"-"+size+"-"+iter+suffix+".csv");
		...
		File file2 = new File(ExamplesUtils.OUTPUT_FOLDER + "rpq/new2/new-"+ver+"-"+size+"-"+iter+suffix+"_mem.csv");
		FileWriter csvWriter2;
		if (file2.exists())
			csvWriter2 = new FileWriter(file2, true);
		else {
			csvWriter2 = new FileWriter(file2);
		...
		String queryType = "";
		if (suffix.equals("-nc")) queryType = "-noconverse";
		else if (suffix.equals("-s1")) queryType = "-star-1";
		else if (suffix.equals("-s2")) queryType = "-star-2-2";
			
		System.out.println("Loading and Parsing Query["+i+"_"+j+"_"+k+"-"+(k+1)+"] "+l+" for size " + size + " in iteration " + iter);
		File queryInput = new File(ExamplesUtils.INPUT_FOLDER + "rpq/newqueries/shop-a-["+i+"_"+j+"_"+k+"-"+(k+1)+"]"+queryType+"/query-"+l+".sparql");
