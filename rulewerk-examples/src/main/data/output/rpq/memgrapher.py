...
ylabels = ["Query", "Parsing Memory", "Translating Memory", "Reasoning Memory"]
filenames = ["query", "parsem", "transm", "reasonm"]
...
for num in range(expnum):
	with open(f'new-1-{size}-{num+1}_mem.csv', 'r') as reader:
		next(reader)
	...
	with open(f'new-2-{size}-{num+1}_mem.csv', 'r') as reader:
		next(reader)
		for line in reader.readlines():
			header = line.split(',')[0].strip().split('_')
			if header[1] == f'{Dis}' and header[2] == f'{Con}' and int(header[0]) in CRPQ:
	...
	if not Conv:
		with open(f'new-1-{size}-{num+1}-nc_mem.csv', 'r') as reader:
			next(reader)
			for line in reader.readlines():
				header = line.split(',')[0].strip().split('_')
				if header[1] == f'{Dis}' and header[2] == f'{Con}' and int(header[0]) in CRPQ:
					a3[int(header[3])][mapping[header[0]]] += int(line.split(',')[graphOf].strip())
		with open(f'new-2-{size}-{num+1}-nc_mem.csv', 'r') as reader:
			next(reader)
			for line in reader.readlines():
				header = line.split(',')[0].strip().split('_')
				if header[1] == f'{Dis}' and header[2] == f'{Con}' and int(header[0]) in CRPQ:
